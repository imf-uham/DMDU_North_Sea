...
tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/sst_bothrcps.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot() + 
    geom_line(aes(seq(stop_year+1,stop_year_fut-1,1), unlist(sst45[2:length(sst45)])), color = plasma(4)[3], size = 1, alpha = 0.3) + 
    geom_line(aes(seq(stop_year+1,stop_year_fut-1,1), unlist(sst85[2:length(sst85)])), color = plasma(4)[2], size = 1, alpha = 0.3) + 
    geom_line(aes(seq(stop_year+1,stop_year_fut-1,1), 0.5*unlist(sst45[2:length(sst45)]) + 0.5*unlist(sst85[2:length(sst85)])), size = 1) + 
    xlim(c(2030,2100)) + ylim(min(sst$mean_sst), c(max(c(max(sst45), max(sst85))))) + theme_bw() + theme(text = element_text(size = 19)) + xlab('Time [a]') + ylab('SST [°C]')
dev.off()
```
...
```{r}
# 7) Plot the median projected recruitment and SSB for a single policy over projection years for both climate scenarios

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/med_recruitment.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(dat%>%ungroup()%>%filter(metric == 'n_age_1_year' & manage_cat == 'harvest' & manage_val == unique(manage_val[manage_cat == 'harvest'])[4])%>%select(year, RCP, val)%>%group_by(year, RCP)%>%mutate(md_recr = median(val), q25_recr= quantile(val, 0.25, names = F), q75_recr = quantile(val, 0.75, names = F))%>%ungroup()%>%select(-val)%>%distinct()) + 
...
tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/med_ssb.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(dat%>%ungroup()%>%filter(metric == 'ssb_year' & manage_cat == 'harvest' & manage_val == unique(manage_val[manage_cat == 'harvest'])[4])%>%select(year, RCP, val)%>%group_by(year, RCP)%>%mutate(md_ssb = median(val), q25_ssb = quantile(val, 0.25, names = F), q75_ssb = quantile(val, 0.75, names = F))%>%ungroup()%>%select(-val)%>%distinct()) + 
...
dat_a <- dat%>%group_by(manage_cat, manage_val_class, decade_class, year)%>%mutate(
  ...
  ) # calculate sustainability risk etc. (see next step for details) for every year instead of for every decade class only
...
# 10) Plotting risks over harvest levels
...
```{r}
# 11) Plotting temporal risk dynamics

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/sust_risk_catch_annually.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(distinct(dat_a%>%filter(manage_cat == 'catch')%>%select(manage_val_class, year, risk_both_mn))) + geom_line(aes(manage_val_class/1e+3, risk_both_mn*100, color = year, group = year)) + xlab('Catch [kt]') + ylab('Sustainability risk [%]') + scale_color_viridis(direction = -1, 'Year', option = 'plasma', begin = 0.2, end = 0.8) + theme_bw() + theme(text = element_text(size = 15), legend.position = 'bottom', legend.text = element_text(angle = 45, hjust = T)) + ylim(c(0,100))
dev.off()

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/sust_risk_harvest_annually.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(distinct(dat_a%>%filter(manage_cat == 'harvest')%>%select(manage_val_class, year, risk_both_mn))) + geom_line(aes(manage_val_class*100, risk_both_mn*100, color = year, group = year)) + xlab('Harvest rate [%]') + ylab('Sustainability risk [%]') + scale_color_viridis(direction = -1, 'Year', option = 'plasma', begin = 0.2, end = 0.8) + theme_bw() + theme(text = element_text(size = 15), legend.position = 'bottom', legend.text = element_text(angle = 45, hjust = T)) + ylim(c(0,100))
dev.off() # plotting sustainability risk, for each projection year, for projections done with i) fixed catch and ii) fixed harvest rate

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/profit_risk_catch_annually.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(distinct(dat_a%>%filter(manage_cat == 'catch')%>%select(manage_val_class, year, profit_risk_mn))) + geom_line(aes(manage_val_class/1e+3, profit_risk_mn*100, color = year, group = year)) + xlab('Catch [kt]') + ylab('Profitability risk [%]') + scale_color_viridis(direction = -1, 'Year', option = 'plasma', begin = 0.2, end = 0.8) + theme_bw() + theme(text = element_text(size = 15), legend.position = 'bottom', legend.text = element_text(angle = 45, hjust = T)) + ylim(c(0,100))
dev.off()

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/profit_risk_harvest_annually.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(distinct(dat_a%>%filter(manage_cat == 'harvest')%>%select(manage_val_class, year, profit_risk_mn))) + geom_line(aes(manage_val_class*100, profit_risk_mn*100, color = year, group = year)) + xlab('Harvest rate [%]') + ylab('Profitability risk [%]') + scale_color_viridis(direction = -1, 'Year', option = 'plasma', begin = 0.2, end = 0.8) + theme_bw() + theme(text = element_text(size = 15), legend.position = 'bottom', legend.text = element_text(angle = 45, hjust = T)) + ylim(c(0,100))
dev.off() # plotting profitability risk, for each projection year, for projections done with i) fixed catch and ii) fixed harvest rate

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/sust_risk_catch_annually_tile.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(distinct(dat_a%>%filter(manage_cat == 'catch')%>%select(manage_val_class, year, risk_both_mn))) + geom_tile(aes(year, manage_val_class/1e3, fill = risk_both_mn*100)) + xlab('Year') + ylab('Catch [kt]') + scale_fill_viridis('Risk [%]', option = 'plasma', begin = 0.0, end = 1.0) + theme_bw() + theme(text = element_text(size = 15), legend.position = 'bottom', legend.text = element_text(angle = 45, hjust = T))
dev.off()

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/sust_risk_harvest_annually_tile.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(distinct(dat_a%>%filter(manage_cat == 'harvest')%>%select(manage_val_class, year, risk_both_mn))) + geom_tile(aes(year, manage_val_class*100, fill = risk_both_mn*100)) + xlab('Year') + ylab('Harvest rate [%]') + scale_fill_viridis('Sustainability risk [%]', option = 'plasma', begin = 0.0, end = 1.0) + theme_bw() + theme(text = element_text(size = 15), legend.position = 'bottom', legend.text = element_text(angle = 45, hjust = T))
dev.off() # plotting sustainability risk, for each projection year, for projections done with i) fixed catch and ii) fixed harvest rate

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/profit_risk_catch_annually_tile.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(distinct(dat_a%>%filter(manage_cat == 'catch')%>%select(manage_val_class, year, profit_risk_mn))) + geom_tile(aes(year, manage_val_class/1e3, fill = profit_risk_mn*100)) + xlab('Year') + ylab('Catch [kt]') + scale_fill_viridis('Profitability risk [%]', option = 'plasma', begin = 0.0, end = 1.0) + theme_bw() + theme(text = element_text(size = 15), legend.position = 'bottom', legend.text = element_text(angle = 45, hjust = T))
dev.off()

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/profit_risk_harvest_annually_tile.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot(distinct(dat_a%>%filter(manage_cat == 'harvest')%>%select(manage_val_class, year, profit_risk_mn))) + geom_tile(aes(year, manage_val_class*100, fill = profit_risk_mn*100)) + xlab('Year') + ylab('Harvest rate [%]') + scale_fill_viridis('Profitability risk [%]', option = 'plasma', begin = 0.0, end = 1.0) + theme_bw() + theme(text = element_text(size = 15), legend.position = 'bottom', legend.text = element_text(angle = 45, hjust = T))
dev.off() # plotting profitability risk, for each projection year, for projections done with i) fixed catch and ii) fixed harvest rate

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/sust_risk_annually_cumulative.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot() + 
  geom_ribbon(data = distinct(dat_a%>%filter(manage_cat == 'harvest')%>%select(manage_val_class, year, risk_both_mn)%>%group_by(year)%>%mutate(risk_md = median(risk_both_mn), risk_q25 = quantile(risk_both_mn, 0.25), risk_q75 = quantile(risk_both_mn, 0.75))%>%ungroup()%>%select(year, risk_md, risk_q25, risk_q75)%>%distinct()), aes(year, ymin = risk_q25*100, ymax = risk_q75*100), fill = plasma(4)[2], alpha = 0.5) + 
  geom_line(data = distinct(dat_a%>%filter(manage_cat == 'harvest')%>%select(manage_val_class, year, risk_both_mn)%>%group_by(year)%>%mutate(risk_md = median(risk_both_mn))%>%ungroup()%>%select(year, risk_md)%>%distinct()), aes(year, risk_md*100), color = plasma(4)[2], size = 1.5) + 
  geom_ribbon(data = distinct(dat_a%>%filter(manage_cat == 'catch')%>%select(manage_val_class, year, risk_both_mn)%>%group_by(year)%>%mutate(risk_md = median(risk_both_mn), risk_q25 = quantile(risk_both_mn, 0.25), risk_q75 = quantile(risk_both_mn, 0.75))%>%ungroup()%>%select(year, risk_md, risk_q25, risk_q75)%>%distinct()), aes(year, ymin = risk_q25*100, ymax = risk_q75*100), fill = plasma(4)[3], alpha = 0.5) +  
  geom_line(data = distinct(dat_a%>%filter(manage_cat == 'catch')%>%select(manage_val_class, year, risk_both_mn)%>%group_by(year)%>%mutate(risk_md = median(risk_both_mn))%>%ungroup()%>%select(year, risk_md)%>%distinct()), aes(year, risk_md*100), color = plasma(4)[3], size = 1.5) +
  xlab('Year') + ylab('Sustainability risk [%]') + 
  theme_bw() + theme(text = element_text(size = 15))
dev.off() # plotting average sustainability risk (over harvest scenarios) over time

tiff(filename = "/home/jan/PhD/EMA/North_Sea_Paper/profit_risk_annually_cumulative.tiff",
     width = 5,height = 5,compression = "lzw",res = 600, units = "in")
ggplot() + 
  geom_ribbon(data = distinct(dat_a%>%filter(manage_cat == 'harvest')%>%select(manage_val_class, year, profit_risk_mn)%>%group_by(year)%>%mutate(risk_md = median(profit_risk_mn), risk_q25 = quantile(profit_risk_mn, 0.25), risk_q75 = quantile(profit_risk_mn, 0.75))%>%ungroup()%>%select(year, risk_md, risk_q25, risk_q75)%>%distinct()), aes(year, ymin = risk_q25*100, ymax = risk_q75*100), fill = plasma(4)[2], alpha = 0.5) + 
  geom_line(data = distinct(dat_a%>%filter(manage_cat == 'harvest')%>%select(manage_val_class, year, profit_risk_mn)%>%group_by(year)%>%mutate(risk_md = median(profit_risk_mn))%>%ungroup()%>%select(year, risk_md)%>%distinct()), aes(year, risk_md*100), color = plasma(4)[2], size = 1.5) + 
  geom_ribbon(data = distinct(dat_a%>%filter(manage_cat == 'catch')%>%select(manage_val_class, year, profit_risk_mn)%>%group_by(year)%>%mutate(risk_md = median(profit_risk_mn), risk_q25 = quantile(profit_risk_mn, 0.25), risk_q75 = quantile(profit_risk_mn, 0.75))%>%ungroup()%>%select(year, risk_md, risk_q25, risk_q75)%>%distinct()), aes(year, ymin = risk_q25*100, ymax = risk_q75*100), fill = plasma(4)[3], alpha = 0.5) +  
  geom_line(data = distinct(dat_a%>%filter(manage_cat == 'catch')%>%select(manage_val_class, year, profit_risk_mn)%>%group_by(year)%>%mutate(risk_md = median(profit_risk_mn))%>%ungroup()%>%select(year, risk_md)%>%distinct()), aes(year, risk_md*100), color = plasma(4)[3], size = 1.5) +
  xlab('Year') + ylab('Profitability risk [%]') + 
  theme_bw() + theme(text = element_text(size = 15))
dev.off() # plotting average profitability risk (over harvest scenarios) over time
```

```{r}
# 12) Trade-off analysis - sustainability risk vs profitability risk

best_troffs <- dat%>%ungroup()%>%select(manage_cat, decade_class, manage_val_class, risk_both_mn, profit_risk_mn)%>%group_by(manage_cat, decade_class, manage_val_class)%>%summarize(risk_diff = abs(1 - profit_risk_mn/risk_both_mn), add_risk = profit_risk_mn + risk_both_mn)%>%ungroup()%>%group_by(manage_cat, decade_class)%>%mutate(risk_diff = risk_diff/max(risk_diff), add_risk = add_risk/max(add_risk))%>%mutate(both_risk = risk_diff + add_risk)%>%ungroup()%>%group_by(manage_cat, decade_class)%>%filter(both_risk == min(both_risk))%>%distinct() # calculate the policy levels associated with "best trade-off", i.e. the level of intended catch and intended harvest rate yielding the lowest additive risks (sustainability- and profitability risks) and yielding the lowest difference from a ratio of 1 between the two risk types (criteria are added to determine exploitation level of best trade-off)

best_troffs <- best_troffs%>%ungroup()%>%select(-c(add_risk, risk_diff, both_risk))%>%rename(best_manage = manage_val_class)
...
```{r}
# 13) Plotting realized harvest vs intended harvest (for cases where stock is overfished so it can't sustain the intended level of catch / harvest rate anymore)
...
```{r}
# 14) Plots for scenario discovery
...
p1 <- p1 + theme_linedraw() + guides(color = guide_legend(override.aes = list(alpha = 1)))
p1_cp <- p1 + theme(legend.position = c(0.85,0.4))

p1a <- ggMarginal(p1_cp, margins = 'x', type="density", xparams = list(  bins=100), fill="grey",
                  col="black")
...
p2a <- ggMarginal(p2, margins = 'x', type="density", xparams = list(  bins=100), fill="grey",
                  col="black")
# the same for fixed catch and the Ricker model
...
p3a <- ggMarginal(p3, margins = 'x', type="density", xparams = list(  bins=100), fill="grey",
                  col="black")
# the same for fixed harvest rate and the Beverton-Holt model
...
p1 <- p1 + theme_linedraw() + guides(color = guide_legend(override.aes = list(alpha = 1)))
p4 <- p1 + theme(legend.position = 'none')

p4a <- ggMarginal(p4, margins = 'x', type="density", xparams = list(  bins=100), fill="grey",
                  col="black")
# the same for fixed harvest rate and the Ricker model
...
```{r}
# 15) For feature-scoring analysis, see separate Python script
```
