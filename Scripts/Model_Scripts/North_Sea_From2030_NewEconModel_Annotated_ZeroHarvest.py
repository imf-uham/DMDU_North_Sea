...
"""
robjects.r['load']("/home/jan/PhD/Data/Physical_Data/tot_sst.RData")
...
files = os.listdir('/home/jan/PhD/Data/Physical_Data/Projections/Henrike_Thuenen_Historical_Extracted_AllVars/')
files = np.sort(files).tolist()
temp_list = []
for i in range(len(files)):
    robjects.r['load']('/home/jan/PhD/Data/Physical_Data/Projections/Henrike_Thuenen_Historical_Extracted_AllVars/'+files[i])
...
sst45 = pd.read_csv('/home/jan/PhD/Data/Physical_Data/Projections/Henrike_Thuenen_RCP45_Extracted/EnvData_Henrike_NS_RCP45.csv') 
...
sst85 = pd.read_csv('/home/jan/PhD/Data/Physical_Data/Projections/Henrike_Thuenen_RCP85_Extracted/EnvData_Henrike_NS_RCP85.csv')
...
"""     
f_old = pd.read_csv('/home/jan/PhD/Data/Lindegren_Papers/Cod_North_Sea/2021_Assessment/FatAge.txt', sep = '	') # natural mortality per age-class and year
...
m_old = pd.read_csv('/home/jan/PhD/Data/Lindegren_Papers/Cod_North_Sea/2021_Assessment/MatAge.txt', sep = '	') # natural mortality per age-class and year
...
wt_old = pd.read_csv('/home/jan/PhD/Data/Lindegren_Papers/Cod_North_Sea/2021_Assessment/WatAge.txt', sep = '	') # weight in the stock per age-class and year
...
wt_ct_old = pd.read_csv('/home/jan/PhD/Data/Lindegren_Papers/Cod_North_Sea/2021_Assessment/WatAge_Catch.txt', sep = '	') # weight in the catch  
...
mt_old = pd.read_csv('/home/jan/PhD/Data/Lindegren_Papers/Cod_North_Sea/2021_Assessment/PatAge.txt', sep = '	') # maturity rates per age-class and year
...
numbers = pd.read_csv('/home/jan/PhD/Data/Lindegren_Papers/Cod_North_Sea/2021_Assessment/NatAge.txt', sep = '	') # numbers of fish in the stock per age-class 
# and year
...
"""
catch_old_full = pd.read_csv('/home/jan/PhD/Data/Lindegren_Papers/Cod_North_Sea/2021_Assessment/Catch_AY.txt', sep = '	') # catch in numbers per age-class 
# and year
catch_old_full = np.transpose(np.array(catch_old_full))
catch_old_full = catch_old_full[1:,:]
    
catch_tot = pd.read_csv('/home/jan/PhD/Data/Lindegren_Papers/Cod_North_Sea/2021_Assessment/Catch.txt', sep = '	') # total catches (in weight)
...
Schenk et al. (2023). Weight- and price classes and parameter values were also taken from Schenk et al. (2023)
"""
...
co = 1.21
tau = 0.0107 # parameters of the cost function (see Schenk et al., in prep.)
...
def make_past_profit(): # function for calculating past profit
    ...
    # ct = co * np.exp(tau * (years - 2019)) * 1000
    cost = co * catch_tot * 1000 # calculating past cost
    
    profit = (np.sum(revenue, axis = 0) - cost) # calculating past profit. We assume 2019 prices for all years and do not consider cost development
    # over time, hence these modeled past profits are a simplified approximation
    
    return(profit)

profit_past = make_past_profit() # calculating past profits
past_profit = np.mean(profit_past[np.arange(1963,2019) >= 2000]) # determining the average profit of years 2000-2018 (used as custom economic reference point for calculation 
...
def cod_problem(
    ...
    for t in range(1,len(sst)): # looping over the projection yeats
        ...
        # ct = co * np.exp(tau * (((2029-2018) + t-1) - (2019-2018))) # calculating cost of fishing, which depends on realized catch 
        # # and temporal dynamics (e.g. increasing labour costs (Schenk et al., 2023)). Cost function is parameterized with 2019 as baseline year, 
        # # hence the factor multiplied with tau must be 10 for the first projection year (initially 11 = 2029-2019)
        cost = co * (rl_catch_w[t-1]) * 1000 # calculating cost of fishing, which depends on catch and a baseline cost factor (co) (Schenk et al., 2023)
...
"""
9) Load the SR parameterizations and climate scenarios from the model runs conducted 
with various fixed catches / fixed harvest rates, so we can now test them under 
zero catch / zero harvest rate
"""
sr_params = pd.read_csv("/home/jan/PhD/EMA/North_Sea_Paper/Catch_Ricker_Params.csv")

alphas = np.array(sr_params['a_rk'])
# alphas = np.array(sr_params['a_bh'])
betas = np.array(sr_params['b_rk'])
# betas = np.array(sr_params['b_bh'])
gammas = np.array(sr_params['c_rk'])
# gammas = np.array(sr_params['c_bh'])

ssts_inds = np.zeros(len(alphas))
ssts_inds[sr_params['sst'] == 'sst45'] = 0
ssts_inds[sr_params['sst'] == 'sst85'] = 1
ssts = np.zeros([len(alphas), len(sst45)])
ssts[ssts_inds == 0,:] = sst45
ssts[ssts_inds == 1,:] = sst85

"""
10) Run the model with zero catch / zero harvest rate and the SR parameterizations and
climate scenarios loaded in the step before
"""
def make_preds(i):
    results = cod_problem(a_rk = alphas[i], b_rk = betas[i], c_rk = gammas[i],
                       catch_mtp = 0, metric_choice = 'catch',
                       sr_choice = 'rk', sst = np.array(['sst45','sst85'])[int(ssts_inds[i])], 
                       full_output = 'yes')
    
    # results = cod_problem(a_rk = alphas[i], b_rk = betas[i], c_rk = gammas[i],
    #                     hrv_rate = 0, metric_choice = 'harvest',
    #                     sr_choice = 'rk', sst = np.array(['sst45','sst85'])[int(ssts_inds[i])], 
    #                     full_output = 'yes')
    
    # results = cod_problem(a_bh = alphas[i], b_bh = betas[i], c_bh = gammas[i],
    #                     catch_mtp = 0, metric_choice = 'catch',
    #                     sr_choice = 'bh', sst = np.array(['sst45','sst85'])[int(ssts_inds[i])], 
    #                     full_output = 'yes')
    ...
    results = np.concatenate([np.zeros([1,]), 
    ...
    if i in np.arange(1000, len(alphas), 100):
        print(i)
    return(results)  

with Pool() as p:
    time_series = p.map(make_preds, range(len(alphas))) # performing the second round of model runs via parallel processing
...
time_series.to_csv('/home/jan/PhD/EMA/Catch_Ricker_ZeroHarvest.csv') # saving the stock metrics to disk
...
# Canada, 191, 382
# - Schenk, H., Zimmermann, F. & Quaas, M. (2023). The economics of reversing fisheries-induced evolution. Nature Sustainability
